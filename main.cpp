...
void KnightTour(myQueue<string> &Tour, string begin);
vector<string> availablePos(const myQueue<string> &Tour, const string &square, const string &checked);
vector<int> degree (const myQueue<string> &Tour, vector<string> pos) ;
...
void Show (const myQueue<string> &Tour);
void PrintTour(ofstream &out, const myQueue<string> &Tour);
...
void KnightTour(myQueue<string> &Tour, string begin)
{
    ...
    while (Tour.size() < 64)
    {
        ...
        {
            checked = "";
            Tour << (square = Choose (pos,deg));
        }
    }
}


vector<string> availablePos(const myQueue<string> &Tour, const string &square, const string &checked) // gives avialble position for given square
{
    vector<string> pos;
...
vector<int> degree (const myQueue<string> &Tour, vector<string> pos) //gives degree for given square
{
    vector<int> deg;
    string checked = "";
...
void closed(const string &begin, const string &end)
{
    if ((abs(begin[0]-end[0]) == 2 && abs(begin[1]-end[1]) == 1)
        ...
        cout << endl<<"The Knight Tour started from " << begin << " and ended in " << end << endl
